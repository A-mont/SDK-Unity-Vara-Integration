...
public class Transfer : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        if (_clientvara != null && _clientvara.IsConnected)
        {
            ...
            Debug.Log($"Extrinsic submitted : {transfer}");
            Console.WriteLine($"Transaction : {transfer}");


            // Enviar la transacción
            uint lifetime = 64; // Lifetime in blocks

            Hash extrinsic = await _clientvara.Author.SubmitExtrinsicAsync(transfer, Alice, ChargeTransactionPayment.Default(), lifetime, CancellationToken.None);
