...
public class TransferAll : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        if (_clientvara != null && _clientvara.IsConnected)
        {
            ...
            Debug.Log($"Extrinsic submitted : {transferAll}");
            Console.WriteLine($"Transaction : {transferAll}");


            // Enviar la transacción
            uint lifetime = 64; // Lifetime in blocks

            Hash extrinsic = await _clientvara.Author.SubmitExtrinsicAsync(transferAll, Alice, ChargeTransactionPayment.Default(), lifetime, CancellationToken.None);
